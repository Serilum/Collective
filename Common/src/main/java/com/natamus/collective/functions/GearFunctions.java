...
public class GearFunctions {
	...
	public static void setInventoryFromGearString(Level level, Inventory inventory, String gearString) {
		for (String line : getGearStringSplit(gearString)) {
			...
			setEquipmentSlot(inventory, getEquipmentSlotFromSlotString(slotString), itemStack);
		}
	}

	public static void setEquipmentSlot(Inventory inventory, EquipmentSlot equipmentSlot, ItemStack itemStack) {
		inventory.equipment.set(equipmentSlot, itemStack);
	}

	public static EntityEquipment getEntityEquipment(Player player) {
		return getEntityEquipment(player.getInventory());
	}
	public static EntityEquipment getEntityEquipment(Inventory inventory) {
	...
	public static String getGearStringFromPlayer(Player player, boolean parseActiveEffects) {
		...
		if (parseActiveEffects) {
			...
			if (!activeEffectsMap.isEmpty()) {
				...
				List<String> effectIdentifierList = new ArrayList<>();
				...
				for (Holder<MobEffect> mobEffectHolder : activeEffectsMap.keySet()) {
					...
					Identifier Identifier = mobEffectRegistry.getKey(mobEffectHolder.value());
					if (Identifier == null) {
						continue;
					}

					String rlString = Identifier.toString();
					int amplifier = mobEffectInstance.getAmplifier()+1;
					int duration = mobEffectInstance.getDuration();

					effectIdentifierList.add(rlString);
					effectLineData.put(rlString, rlString + ";lvl:" + amplifier + ";duration:" + duration);
				}

				Collections.sort(effectIdentifierList);

				for (String rlString : effectIdentifierList) {
					String lineData = effectLineData.get(rlString);

					if (!effectsStringBuilder.toString().isEmpty()) {
	...
	private static ItemStack getItemStackFromGearStringLine(Level level, String line) {
		...
		try {
			...
			itemStack = ItemStack.CODEC.parse(level.registryAccess().createSerializationContext(NbtOps.INSTANCE), compoundTag).result().orElse(ItemStack.EMPTY);
	...
	private static List<MobEffectInstance> getEffectsFromLine(Registry<MobEffect> mobEffectRegistry, String line) {
		...
		for (String effectRaw : effectData.split("\\|")) {
			...
			Optional<Holder.Reference<MobEffect>> optionalMobEffectReference = mobEffectRegistry.get(Identifier.parse(rlString));
