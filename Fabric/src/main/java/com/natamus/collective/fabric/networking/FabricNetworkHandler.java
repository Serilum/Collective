...
public class FabricNetworkHandler extends PacketRegistrationHandler {
    ...
    protected <T> void registerPacket(PacketContainer<T> container) {
        ...
        ServerPlayNetworking.registerGlobalReceiver(container.getType(), (ServerPlayNetworking.PlayPayloadHandler<CommonPacketWrapper<T>>) (payload, context) -> context.player().getServer().execute(() -> container.handler().accept(new PacketContext<>(context.player(), payload.packet(), side))));
